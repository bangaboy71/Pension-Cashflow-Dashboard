...
# 2. 데이터 로드 및 '무적' 정제 레이어
conn = st.connection("gsheets", type=GSheetsConnection)

try:
    # 캐시를 무시하고 최신 데이터를 읽어옵니다.
    df = conn.read(ttl=0)
    
    # [에러 해결 핵심] 컬럼명에서 유령 문자(BOM)와 공백을 완전히 제거합니다.
...
# 3. 사적 자산 필터링 (공적연금 제외)
# '계좌 유형' 컬럼을 안전하게 참조합니다.
target_col = '계좌 유형'
if target_col in df.columns:
    private_assets = df[df[target_col].isin(['IRP', 'ISA', '일반'])].copy()
else:
    st.error("'계좌 유형' 컬럼을 찾을 수 없습니다. 시트의 첫 번째 열 제목을 확인해 주세요.")
    st.stop()
...
# 5. 메인 화면 구성
st.title("🛡️ 현금흐름 고도화 관제탑")
st.subheader(f"현금흐름 목표: 월 {TARGET_PRIVATE/10000:,.0f}만 원")
...
with tab1:
    ...
    st.info("💡 [계좌 유형 > 투자성격 > 종목명] 순으로 자산 분포를 확인합니다.")
    # Sunburst Chart 구현
    fig_sun = px.sunburst(
        private_assets, 
        path=['계좌 유형', '투자성격', '종목명'], 
...
with tab4:
    ...
    with col_l:
        ...
        fig_tax = px.pie(tax_df, values='투자원금', names='세금성격', hole=0.5, title="자산 세금 성격 비중")
