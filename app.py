...
with st.expander("🔍 보유 종목별 6개월 주가 추이 및 리스크 판정"):
    ...
    for i, (idx, row) in enumerate(private_assets.iterrows()):
        with t_cols[i]:
            ...
            st.markdown(f"""<div class="strategy-card" style="border: 1px solid {clr}44;"><div style="color: {clr}; font-size: 1.1rem; font-weight: bold;">{sig}</div><div style="font-size: 0.8rem; margin-top:5px; color:#999;">비중: {(row['현재 가치']/private_assets['현재 가치'].sum()*100):.1f}%</div></div>""", unsafe_allow_html=True)

# ---------------------------------------------------------
# 6. [핵심 변경] 하단 횡열 병렬 배치 (3 Columns)
# ---------------------------------------------------------
st.markdown("<br>", unsafe_allow_html=True)
...
with col_b:
    st.markdown("<div class='section-title'>🌊 수입 폭포 (기여도)</div>", unsafe_allow_html=True)
    fig_water = go.Figure(go.Waterfall(
        measure = ["relative"] * len(private_assets) + ["total"],
        x = list(private_assets['종목명']) + ["총 합계"],
...
with col_c:
    st.markdown("<div class='section-title'>📅 입금 일정 (달력)</div>", unsafe_allow_html=True)
