...
# ---------------------------------------------------------
# 3. 시장 지수 수집 함수 (가족 관제탑 로직)
# ---------------------------------------------------------
@st.cache_data(ttl=600)
def get_market_status():
    data = {
        "KOSPI": {"val": "-", "pct": "0.00%", "color": "#777"},
        "KOSDAQ": {"val": "-", "pct": "0.00%", "color": "#777"},
        "USD/KRW": {"val": "-", "pct": "0원", "color": "#777"},
        "VOLUME": {"val": "-", "pct": "천주", "color": "#777"}
    }
    ...
    try:
        ...
        for code in ["KOSPI", "KOSDAQ"]:
            ...
            diff_txt = soup.select_one("#change_value_and_rate").get_text(" ", strip=True)
            for word in ["상승", "하락", "보합"]: diff_txt = diff_txt.replace(word, "")
            data[code]["pct"] = diff_txt
            if "+" in diff_txt: data[code]["color"] = "#FF4B4B"
            elif "-" in diff_txt: data[code]["color"] = "#007BFF"
            if code == "KOSPI":
        ...
        data["USD/KRW"]["color"] = "#FF4B4B" if "+" in sign else "#007BFF"
    except: pass
    return data
...
with t2:
    ...
    fig_water.update_layout(title="사적 자산 수입 엔진 (340만 원 목표)", template="plotly_white")
