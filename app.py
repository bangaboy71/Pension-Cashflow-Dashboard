...
try:
    ...
    # 숫자형 데이터 세척 (쉼표, 원 등 제거 후 숫자로 변환)
    numeric_cols = ['투자원금', '현재 가치', '목표인출액', '매입단가', '목표가', '수량', '수익률(%)']
    for col in numeric_cols:
        if col in df.columns:
            df[col] = df[col].astype(str).str.replace(',', '').str.replace('원', '').str.replace('%', '').str.strip()
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
        elif col == '수익률(%)': # 시트에 컬럼이 없을 경우 대비
            df['수익률(%)'] = 5.0
...
private_assets['종목명'] = private_assets['종목명'].str.strip() # 공백 제거로 매칭 오류 방지
TARGET_PRIVATE = 3400000

# ---------------------------------------------------------
# 3. [시뮬레이션 엔진] 사이드바 설정 (움직이는 즉시 반영됨)
# ---------------------------------------------------------
with st.sidebar:
    st.markdown("### 📉 수익률 시뮬레이션")
    st.info("슬라이더를 조절하면 중앙 지표가 즉시 변합니다.")
...
# ---------------------------------------------------------
# 4. [계산 레이어] 시뮬레이션 결과 산출
# ---------------------------------------------------------
sim_assets = private_assets.copy()

# 슬라이더에서 받은 수익률로 '목표인출액' 실시간 재계산
# 공식: (현재가치 * 수익률 / 100) / 12개월
...
actual_total_income = private_assets['목표인출액'].sum() # 시트의 원본 수치

# ---------------------------------------------------------
# 5. 시장 지표 엔진 (가족 관제탑 스타일)
# ---------------------------------------------------------
